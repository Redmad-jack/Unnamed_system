...
class TestSystemPromptInvariants:
    ...
    def test_expired_identity_candidate_does_not_inject_confirmation_context(self, builder):
        mem = ShortTermMemory(max_turns=10)
        mem.add(ShortTermEntry(
            role="user",
            content="继续说吧。",
            ...
            metadata={
                "identity_session": {
                    "waiting_for_identity_confirmation": False,
        ...
        assert "Visitor identity confirmation cue:" not in ctx.system_prompt
        assert "identity_confirmation_context" not in prompt_layer.metadata["partials"]
